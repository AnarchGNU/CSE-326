...
class dbConnection(object):
    ...
    def createMultiRecommendationRow(self, list):
        connection = self.getConnection()
        numberOfRows = -1
        try:
            with connection.cursor() as cursor:
                sql = "INSERT IGNORE INTO `user_based_recommendation` (`user_id`, `score`, `movie_id`) " \
                      "VALUES (%s, %s, %s)"
                numberOfRows = cursor.executemany(sql, list)
            connection.commit()
                ...
                pymysql.ProgrammingError, Exception) as e:
            print(e)
    ...
    def deleteUserBasedRecommendation(self, userID):
        connection = self.getConnection()
        numberOfRows = -1
        try:
            with connection.cursor() as cursor:
                sql = "Delete from user_based_recommendation where user_id = %s"

                numberOfRows = cursor.execute(sql, userID)
            connection.commit()
        except (pymysql.DatabaseError, pymysql.MySQLError, pymysql.Warning, pymysql.DataError, pymysql.Error,
                pymysql.InternalError, pymysql.IntegrityError, pymysql.InterfaceError, pymysql.OperationalError,
                pymysql.ProgrammingError) as e:
    ...
    def selectAllMovieWithGenres(self):
        connection = self.getConnection()
        try:
            with connection.cursor() as cursor:
                sql = "SELECT movie.*, GROUP_CONCAT(movie_genre.genre_name) as movie_genres FROM movie join movie_genre on movie.movie_id = movie_genre.movie_id GROUP BY movie.movie_id"
                result = cursor.execute(sql)
                result = cursor.fetchall()
                return result
        except (pymysql.DatabaseError, pymysql.MySQLError, pymysql.Warning, pymysql.DataError, pymysql.Error,
    ...
    def getByMID(self, movieID):
        ...
        try:
            with connection.cursor() as cursor:
                ...
                result = cursor.execute(sql, str(movieID))
                result = cursor.fetchall()
                return result;
        except (pymysql.DatabaseError, pymysql.MySQLError, pymysql.Warning, pymysql.DataError, pymysql.Error,
                pymysql.InternalError, pymysql.IntegrityError, pymysql.InterfaceError, pymysql.OperationalError,
